...
居间中心性的算法在计算大型网络是非常耗时。NetworkX中的`betweenness_centrality()`方法可以方便的计算出网络个节点的居间中心性。
...
菲利普·博纳西科设计了一套特征向量中心性（eigenvector centrality）算法，可用来检测“灰衣主教”，这套算法在NetworkX中就是`eigenvector_centrality()`方法。

### PageRank

PageRank类似于特征向量中心性，但它的算法在庞大社会网络和网络的时间推移中，扩展性会更好。

NetworkX提供了PageRank算法`pagerank()`。
